...
const Applicants = () => {
  ...
  return (
    <div className="page-shell" style={{ padding: '2rem max(1.5rem, 5vw)', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {/* Header Block with Branded Import Action */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2.5rem', paddingBottom: '1rem', borderBottom: `2px solid ${CBE_GOLD}30`, flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: '#475569', fontSize: '0.95rem', fontWeight: 500, margin: 0 }}>Review, structure, and filter participating lottery candidates</p>
        </div>
        <label id="upload-applicants-btn" style={{ cursor: 'pointer' }}>
          <span className={`btn ${uploading ? 'disabled' : ''}`} style={{
            ...
            {uploading ? (
              <><span className="spin-fast" style={{ display:'inline-block', width:14, height:14, border:'2px solid rgba(255,255,255,0.3)', borderTopColor:'#ffffff', borderRadius:'50%' }} /> Importing List...</>
            ) : (
              <>📤 Import Spreadsheet</>
            )}
          </span>
      ...
      {/* Structured Metric Overview Cards */}
      <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
        {[
          { label: 'Total Candidates', value: stats.total, color: '#a78bfa' },
          { label: 'Pending Draw', value: stats.pending, color: '#64748b' },
          { label: 'Assigned Winners', value: stats.winners, color: CBE_GOLD },
          { label: 'Waitlisted Queue', value: stats.waitlist, color: '#ea580c' },
          ...
          <div key={label} style={{ 
            ...
            <div style={{ color: '#475569', fontWeight: 700, fontSize: '0.825rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{label}</div>
          </div>
        ))}
      </div>

      {/* Advanced Criteria Filter Block */}
      <div style={{ 
        background: '#ffffff', 
        padding: '1.25rem', 
        marginBottom: '2rem', 
        ...
        <input className="form-input" placeholder="Filter by Site..." style={{ flex: '1 1 200px', maxWidth: 260, padding: '0.625rem 1rem', border: '1px solid #cbd5e1', borderRadius: '10px' }}
          value={filters.site} onChange={(e) => setFilters(f => ({ ...f, site: e.target.value }))} />
        <input className="form-input" placeholder="Filter by Area..." style={{ flex: '1 1 200px', maxWidth: 260, padding: '0.625rem 1rem', border: '1px solid #cbd5e1', borderRadius: '10px' }}
        ...
        <select className="form-input" style={{ flex: '1 1 160px', maxWidth: 220, padding: '0.625rem 1rem', border: '1px solid #cbd5e1', borderRadius: '10px', background: '#ffffff' }}
          ...
          <option value="">All Draw Status</option>
          <option value="NONE">Pending Selection</option>
          <option value="WINNER">Winner Assigned</option>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ background: `${CBE_FUCHSIA}08`, borderBottom: `2px solid ${CBE_FUCHSIA}20` }}>
                ...
                <th style={{ padding: '1rem 1.25rem', fontSize: '0.8rem', fontWeight: 700, color: CBE_FUCHSIA, textTransform: 'uppercase' }}>Full Name</th>
                <th style={{ padding: '1rem 1.25rem', fontSize: '0.8rem', fontWeight: 700, color: CBE_FUCHSIA, textTransform: 'uppercase' }}>Target Site</th>
                ...
                <th style={{ padding: '1rem 1.25rem', fontSize: '0.8rem', fontWeight: 700, color: CBE_FUCHSIA, textTransform: 'uppercase' }}>Registered Date</th>
              </tr>
            </thead>
              ...
              {loading ? (
                <tr>
                  <td colSpan={8} style={{ textAlign: 'center', padding: '4rem' }}>
                    ...
                    <p style={{ color: '#475569', fontWeight: 500, margin: 0 }}>Gathering applicant parameters...</p>
                  </td>
                </tr>
              ) : paginated.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ textAlign: 'center', padding: '4rem', color: '#64748b', fontWeight: 500 }}>
                    {applicants.length === 0 ? '📭 Register empty. Upload an administrative Excel file to mount candidates.' : '🔍 No candidates match the current filter parameters.'}
                  </td>
                </tr>
              ) : (
                paginated.map((app, i) => (
                  <tr key={app.id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background 0.15s' }}
                      onMouseEnter={e => e.currentTarget.style.background = '#f8fafc'}
                      onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                    <td style={{ padding: '1rem 1.25rem', color: '#94a3b8', fontWeight: 600 }}>{(page - 1) * PER_PAGE + i + 1}</td>
                    <td style={{ padding: '1rem 1.25rem', fontFamily: 'monospace', fontWeight: 600, color: '#475569' }}>{app.idCode || '—'}</td>
                    <td style={{ padding: '1rem 1.25rem', fontWeight: 700, color: '#0f172a' }}>{app.username}</td>
                    <td style={{ padding: '1rem 1.25rem', color: '#0f172a', fontWeight: 500 }}>{app.site}</td>
                    <td style={{ padding: '1rem 1.25rem', fontWeight: 600, color: '#0f172a' }}>{app.area} m²</td>
                    <td style={{ padding: '1rem 1.25rem', fontWeight: 700, color: CBE_FUCHSIA }}>{app.bedroom ?? 0} BR</td>
                    <td style={{ padding: '1rem 1.25rem' }}><StatusBadge status={app.status} /></td>
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 1.5rem', borderTop: '1px solid #edf2f7', background: '#f8fafc' }}>
            <span style={{ fontSize: '0.85rem', color: '#475569', fontWeight: 500 }}>
              Showing Row {(page - 1) * PER_PAGE + 1} – {Math.min(page * PER_PAGE, applicants.length)} of {applicants.length} Records
            </span>
      ...
      {/* Spreadsheet Format Formatting Guidelines */}
      <div style={{ 
        marginTop: '1.5rem', 
        ...
        <p style={{ margin: 0, fontSize: '0.825rem', color: '#475569', fontWeight: 500, lineHeight: 1.5 }}>
          💡 <strong style={{ color: CBE_FUCHSIA, fontWeight: 700 }}>Spreadsheet Parsing Protocol:</strong> Bulk file headers must directly match the structural mapping parameters of the bank platform system, explicitly formatted as:{' '}
          <code style={{ background: '#ffffff', border: '1px solid #cbd5e1', padding: '2px 6px', borderRadius: 6, color: CBE_FUCHSIA, fontFamily: 'monospace', fontWeight: 700 }}>Id</code>,{' '}
          <code style={{ background: '#ffffff', border: '1px solid #cbd5e1', padding: '2px 6px', borderRadius: 6, color: CBE_FUCHSIA, fontFamily: 'monospace', fontWeight: 700 }}>Name</code>,{' '}
